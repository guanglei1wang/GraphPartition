    ...
    for(auto& v_p: _model->_vars)
    {
        ...
        case double_: {
            if (v->get_type() == var_c) {
                ...
                auto c = _mosek_model->variable(real_var->get_name(), real_var->get_dim(), fusion::Domain::inRange(lb,ub));
                _mosek_vars.push_back(c);
                //std::cout << c->toString() << endl;
            }
            else {
                auto sdp_var = (sdpvar<double>*)v; // convert param_ to an sdp_var. 
                size_t num = sdp_var->_symdim; // add sdp vars with dimension sdp_var->_symdim
...
void MosekProgram::create_mosek_constraints() {
    //size_t idx = 0;
    //size_t nb_inst = 0;
    size_t idx = 0, idx_inst = 0,nb_inst = 0, inst = 0;
    size_t c_idx_inst = 0;
    Constraint* c;
    for(auto& p: _model->_cons) {
        ...
        for (int i = 0; i< nb_inst; i++) {
            ...
            monty::rc_ptr<mosek::fusion::Expression > expr= fusion::Expr::constTerm(poly_eval(c->get_cst(), 0)); // expr is a pointer to the Expression.
...
void MosekProgram::set_mosek_objective() {
  ...
  double const_part = poly_eval(_model->_obj.get_cst());
    monty::rc_ptr< ::mosek::fusion::Expression >  expr= fusion::Expr::constTerm(const_part); // expr is a pointer to the Expression.
